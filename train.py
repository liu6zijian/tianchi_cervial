...
import losses
from data_loader import CervicalDataset, collater, full_collater
...
def train(args):
    ...
    sample_path = [os.path.join(cfg.sample_path, x) for x in os.listdir(cfg.sample_path) if '.npz' in x]
    ...
    train_loader = DataLoader(train_data, batch_size=cfg.batch_size, shuffle=True, drop_last=False, collate_fn=full_collater, num_workers=cfg.num_worker, worker_init_fn=worker_init_fn)
