    ...
    for sample_path in sample_paths:
        ...
        for i in range(x_num):
            for j in range(y_num):
                ...
                transformed_all = torch.cat([transformed_anchors, scores], dim=1)
                ...
                for temp_idx in range(scores.shape[0]):
                    x = int(transformed[temp_idx, 0])
                    y = int(transformed[temp_idx, 1])
                    w = max(int(transformed[temp_idx, 2] - transformed[temp_idx, 0]), 1)
                    h = max(int(transformed[temp_idx, 3] - transformed[temp_idx, 1]), 1)
                    p = float(scores[temp_idx, 0])
...
if __name__ == '__main__':
    ...
    sample_paths = glob(os.path.join(cfg.test_path, "test_2", "*.kfb"))
